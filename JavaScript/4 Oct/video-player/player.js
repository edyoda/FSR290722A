...
$(document).ready(function () {
  var playlistContainer = document.getElementById("playlist-wrapper");
  ...
  function createPlaylistCard(obj) {
    ...
    thumbnail.addEventListener("click", onPlaylistClick);

    return mainDiv;
  }

  for (var i = 0; i < playlistData.length; i++) {
    var playlistItem = playlistData[i];
  ...
  function onPlaylistClick(e) {
    ...
    var id = e.target.id;
    for (var j = 0; j < videoPlaySectionData.length; j++) {
      var playSectionObj = videoPlaySectionData[j];
      console.log(playSectionObj);
      if (id == playSectionObj.id) {
        // Check the clicked Id in playSectionData array
        player.src = "https://player.vimeo.com/video/" + playSectionObj.vimeoId;
        // ToDo : Update rest details of the video getting played (on left section)
      }
    }
  }
